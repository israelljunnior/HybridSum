...
function  TF(objectWord, objectSentence, i){
    ...
    if(objectSentence.sentence.toUpperCase().match("\\b"+objectWord.word.toUpperCase()+"\\b", 'g') != null){
                
                
        return Math.log(1+objectWord.frequenceSen[i]);       
...
function IDF(objectWord){
    ...
    return Math.log(sentenceWithObjectWord);
    ...
    for(i = 0; i < arrayObjectSentence.length; i++){
        arrayObjectSentence[i].tf_idf = 0;
           ...
           for(j = 0; j < arrayObjectWord.length; j++){
            ...
            arrayObjectWord[j].tf_idf.push(arrayObjectSentence.length / (TF(arrayObjectWord[j], arrayObjectSentence[i], i) * IDF(arrayObjectWord[j])));
